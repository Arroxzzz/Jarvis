...
_IDLE_TIMEOUT_SEC = 600  # 10 min sem uso → fecha a sessão de automação (libera RAM do Chromium)
...
class _SessionRegistry:
    ...
    def _ensure_watchdog(self) -> None:
        ...
        threading.Thread(target=self._watchdog_loop, daemon=True, name="BrowserIdleWatchdog").start()
        ...
        while True:
            ...
            with self._lock:
                idle = [n for n, s in self._sessions.items()
                        if (now - getattr(s, "_last_used", now)) > _IDLE_TIMEOUT_SEC]
            for name in idle:
                print(f"[Registry] 💤 '{name}' ocioso há {_IDLE_TIMEOUT_SEC//60} min — encerrando.")
                self.close_one(name)
