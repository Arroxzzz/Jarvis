...
def screen_process(
    ...
    angle = (params.get("angle", "screen") or "screen").lower().strip()
        ...
        if angle == "camera":
            ...
            if player and hasattr(player, "start_camera_stream"):
                try:
                    ...
                    print(f"[Vision] ⚠️  Camera stream failed: {exc}")
        else:
    ...
    print(f"[Vision] ▶ angle={angle!r} question='{user_text[:80]}' captured={len(image_bytes):,} bytes")
    return True
