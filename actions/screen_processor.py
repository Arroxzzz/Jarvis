...
_VISION_IDLE_TIMEOUT_SEC = 300  # 5 min sem análises → encerra a sessão de visão (libera WS + thread)

class _IdleShutdown(Exception):
    """Sinaliza encerramento por inatividade — não é um erro real."""
...
class _VisionSession:
    ...
    def _run_event_loop(self) -> None:
        ...
        with _session_lock:
            _session_up = False
        print("[Vision] 💤 Sessão encerrada por inatividade — será recriada no próximo uso.")
    ...
    async def _session_loop(self) -> None:
        ...
        while not self._stop_evt.is_set():
            ...
            except* _IdleShutdown:
                print(f"[Vision] 💤 Sem uso há {_VISION_IDLE_TIMEOUT_SEC//60} min — liberando recursos.")
            except* Exception as eg:
                for exc in eg.exceptions:
                    if not isinstance(exc, _IdleShutdown):
    ...
    async def _idle_watchdog(self) -> None:
        while True:
            ...
            if self._stop_evt.is_set():
                return
            if time.monotonic() - self._last_used > _VISION_IDLE_TIMEOUT_SEC:
                self._stop_evt.set()
                raise _IdleShutdown()
