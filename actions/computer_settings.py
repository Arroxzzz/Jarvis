...
- Return ONLY the JSON, no explanation, no markdown."""
    ...
    try:
        resp_text = resilient_text_call(prompt, task_type="general")
...
def computer_settings(
    ...
    # ── Correção do bug de volume ──────────────────────────────────────────
    # A descrição da tool (em main.py) anuncia "volume" como algo que esta
    # ferramenta controla, mas o ACTION_MAP só tem "volume_up"/"volume_down"
    # (e "volume_set" é tratado à parte, abaixo). Um action=="volume" puro
    # (ou "sound"/"som"/"audio") caía em "Unknown action" e não fazia nada —
    # exatamente o "abaixa o volume um pouco" que você testou. Resolvido por
    # palavra-chave local, sem custo de rede extra (mantém a latência baixa).
    if action in ("volume", "sound", "som", "audio", "loudness"):
        phrase   = f"{raw_action} {description}".lower()
        _down_kw = ("abaix", "diminu", "reduz", "baix", "down", "lower", "less", "menos")
        _up_kw   = ("aument", "sobe", "sobre", "up", "higher", "raise", "more", "mais")
        if value is not None:
            action = "volume_set"
        elif any(k in phrase for k in _down_kw):
            action = "volume_down"
        elif any(k in phrase for k in _up_kw):
            action = "volume_up"
        else:
            return "Should I raise or lower the volume, sir? Or tell me a target percentage."
